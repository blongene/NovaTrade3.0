#!/usr/bin/env python3
"""
Inject a single Coinbase Advanced dryrun trade command into the NovaTrade Bus outbox.

Usage:
  python tools/inject_coinbase_dryrun.py --base-url https://<bus> --secret <OUTBOX_SECRET> \
    --agent edge-primary --symbol BTC/USDC --side buy --amount 5

Notes:
- amount is QUOTE sizing by default (flags=["quote"]).
...
def main():
    ...
    body = {
        ...
        "command": cmd,
        "meta": {"source": "inject_coinbase_dryrun.py", "ts": now},
    }
