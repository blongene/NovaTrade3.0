...
m AS (
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN event='SEEN' AND ts >= NOW()-INTERVAL '7 days'
      THEN (ts AT TIME ZONE 'UTC')::date END) AS distinct_seen_days_7d,
...
gates AS (
  SELECT
    ...
    -- Gate A: memory maturity (tuneable thresholds)
    CASE WHEN b.seen_7d >= 5 AND b.distinct_seen_days_7d >= 3 AND b.confirmed_30d >= 2 AND b.expired_30d = 0
         THEN 1 ELSE 0 END AS gate_A_memory_maturity,
    ...
    CASE WHEN b.last_seen_ts IS NOT NULL AND b.last_seen_ts >= NOW() - INTERVAL '7 days'
         THEN 1 ELSE 0 END AS gate_C_fresh_enough,

    -- Gate D: policy safety (placeholder until policy blocks are logged per token)
    0 AS gate_D_policy_clear,
    'UNKNOWN'::text AS gate_D_note,

    -- Gate E: capital discipline (always preview-only; never enables execution here)
...
stage AS (
  SELECT
    ...
    CASE
      ...
      THEN 1 ELSE 0 END AS ready_preview
  FROM gates g
)
SELECT
  token,
  ...
  -- key metrics
  seen_24h, seen_7d, distinct_seen_days_7d,
  confirmed_7d, confirmed_30d,
  watch_7d,
  expired_30d, demoted_30d,
  last_seen_ts, age_since_last_seen,
  last_source,
  novelty_reason,

  -- gates
  gate_A_memory_maturity,
  gate_B_venue_feasible, gate_B_note,
  gate_C_fresh_enough,
  gate_D_policy_clear, gate_D_note,
