#!/usr/bin/env python3
"""
Inject a single BINANCEUS SELL command into the NovaTrade Bus outbox.

This injector uses BASE sizing (amount_base) for SELL, which is the safest and most deterministic.

Usage:
  python tools/inject_binanceus_sell_base.py --base-url https://<bus> --secret <OUTBOX_SECRET> \
    --agent edge-primary --symbol BTCUSDT --amount-base 5e-05
...
def main():
    ...
    ap.add_argument("--venue", default="BINANCEUS")
    ...
    body = _make_body(args.agent, cmd, "inject_binanceus_sell_base.py", now)
