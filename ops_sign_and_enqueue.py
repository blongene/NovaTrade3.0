...
def _attempt_raw(url: str, secret: bytes, body_dict: Dict[str, Any], *, timeout: float = 15.0,
                 verbose: bool = True) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]], Optional[int]]:
    """
    ...
    for label, headers in _trial_signatures(secret, raw, ts):
        try:
            ...
            if verbose:
                # Keep existing print behaviour for CLI
                if not is_json:
...
def _derive_base_from_env() -> str:
    ...
    If OPS_ENQUEUE_URL already includes /ops/enqueue, chop it off.
    """
...
def _shape_body_from_envelope(envelope: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "payload" in envelope and "type" in envelope:
        # Assume already in {agent_id, type, payload} schema.
        body = {
            "agent_id": agent_id,
            "type": envelope["type"],
    ...
    # Default to 'order.place' for trading commands
...
def attempt(envelope: Dict[str, Any], *, timeout: float = 15.0) -> Dict[str, Any]:
    ...
    # If server returns a structured JSON with its own reason, honor it.
...
# ---------- CLI entrypoint (existing behaviour) ----------
